"""Change patient fields to bool

Revision ID: 98d8176baad3
Revises: c9ab5231c7f3
Create Date: 2024-03-18 20:47:35.827819
...
def upgrade() -> None:
    ...
    op.alter_column("patient_records", "visit", existing_type=sa.VARCHAR(), nullable=False)
    op.drop_column("patient_records", "dep")
    op.drop_column("patient_records", "ischemia")
    op.drop_column("patient_records", "bp")
    op.add_column("patients", sa.Column("bp", sa.Boolean(), server_default=sa.text("false"), nullable=False))
    op.add_column(
        "patients", sa.Column("ischemia", sa.Boolean(), server_default=sa.text("false"), nullable=False)
    )
    op.add_column("patients", sa.Column("dep", sa.Boolean(), server_default=sa.text("false"), nullable=False))
...
def downgrade() -> None:
    ...
    op.alter_column("patient_records", "visit", existing_type=sa.VARCHAR(), nullable=True)
    # ### end Alembic commands ###
